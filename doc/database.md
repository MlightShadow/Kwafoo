...
def migrate_add_ai_processed():
    ...
    if 'ai_processed' not in columns:
        cursor.execute('ALTER TABLE news ADD COLUMN ai_processed BOOLEAN DEFAULT 0')
        conn.commit()
        print("字段ai_processed已添加")
    else:
        print("字段ai_processed已存在")
